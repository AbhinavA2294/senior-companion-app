...
import { CheckInPanel } from "@/components/visits/check-in-panel";
import { CheckOutPanel } from "@/components/visits/check-out-panel";
import { IncidentReportForm } from "@/components/visits/incident-report-form";
...
export default async function CompanionAssignmentDetailPage({ params }: Props) {
  ...
  const { data: cp } = await supabase.from("companion_profiles").select("id, verification_status").eq("profile_id", profile.id).single();

  const { data: assignment } = await supabase
    .from("booking_assignments")
    .select("id, status, assigned_at, responded_at, decline_reason, booking_id")
    .eq("id", params.id)
  ...
  const { data: bookingData } = await supabaseAdmin
    .from("bookings")
    .select("id, status, scheduled_date, scheduled_start_time, duration_hours, location_description, destination_address, special_notes, activity_type:activity_types(name, description)")
  ...
  const statusVariant = assignment.status === "accepted" ? ("success" as const) : assignment.status === "declined" ? ("secondary" as const) : ("warning" as const);

  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <Link href="/companion/bookings" className="text-sm text-sage-600 hover:underline mb-2 inline-block">← Back to booking requests</Link>
        <h1 className="font-display text-senior-3xl font-bold text-gray-900">{booking?.activity_type?.name ?? "Companion Visit"}</h1>
        <Badge variant={statusVariant} className="capitalize mt-1">{(assignment.status as string).replace("_", " ")}</Badge>
      </div>

      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-3"><CardTitle className="text-senior-base">Visit Details</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          {booking?.activity_type?.description && <p className="text-sm text-gray-600">{booking.activity_type.description}</p>}
      ...
      {isPending && (
        <Card className="border-0 shadow-sm">
          <CardHeader className="pb-3"><CardTitle className="text-senior-base">Your Response</CardTitle></CardHeader>
          <CardContent><AssignmentActions assignmentId={params.id} isApproved={isApproved} /></CardContent>
        </Card>
      )}

      {assignment.status === "accepted" && bookingStatus === "accepted" && (
        <CheckInPanel
          bookingId={booking?.id ?? ""}
          scheduledDate={booking?.scheduled_date ? formatDate(booking.scheduled_date) : ""}
          scheduledTime={booking?.scheduled_start_time ? formatTime(booking.scheduled_start_time) : ""}
        />
      )}

      {assignment.status === "accepted" && bookingStatus === "in_progress" && (
        <CheckOutPanel bookingId={booking?.id ?? ""} />
      )}

      {booking?.id && (
        <Card className="border-0 shadow-sm">
          <CardHeader><CardTitle className="text-senior-base">Report an Incident</CardTitle></CardHeader>
          <CardContent>
